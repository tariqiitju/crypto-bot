...
namespace bot
{
    namespace ds
    {
        ...
        public:
            ...
            _real min();
            _real max();
            void clear(){root->recursiveDelete();};
        };
        template<typename _real> 
        ...
        template<typename _real> 
        _real TrieSet<_real>::min(){
            return root->count ? find_by_order(1) : -1.0;
        }
        
        template<typename _real> 
        _real TrieSet<_real>::max(){
            return root->count ? find_by_order(root->count) : -1.0;
        }
        ...
        template<typename _real> 
        _real TrieSet<_real>::median(){
            if(root->count == 0) return -1.0;
            if(root->count & 1) {
                return find_by_order(root->count / 2 + 1);
            }
            return (find_by_order(root->count / 2 + 1) + find_by_order(root->count / 2 ) ) / 2;
        }
